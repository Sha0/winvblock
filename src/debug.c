...
VOID STDCALL DebugIrpEnd ( IN PIRP Irp, IN NTSTATUS Status )
{
	...
	if ( ( Record = IrpListRecord ( Irp ) ) == NULL ) {
		DbgPrint
		    ( "Irp not found in IrpList!! (returned 0x%08x, Status)\n" );
		return;
	}
			 ...
			 IN PCHAR DebugMessage )
{
	...
	case IRP_MJ_SYSTEM_CONTROL:
		sprintf ( DebugMessage, "%s %s", DebugMessage,
			  SystemControlMinorFunctionString ( Stack->
							     MinorFunction ) );
		break;
	case IRP_MJ_PNP:
		sprintf ( DebugMessage, "%s %s", DebugMessage,
		...
		case IRP_MN_QUERY_ID:
			sprintf ( DebugMessage, "%s %s", DebugMessage,
				  QueryIdString ( Stack->Parameters.QueryId.
						  IdType ) );
			break;
	...
	case IRP_MJ_DEVICE_CONTROL:
		sprintf ( DebugMessage, "%s (0x%08x) %s", DebugMessage,
			  ( int )Stack->Parameters.DeviceIoControl.
			  IoControlCode,
