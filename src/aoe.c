...
NTSTATUS STDCALL ZwWaitForSingleObject ( IN HANDLE Handle, IN BOOLEAN Alertable,
					 IN PLARGE_INTEGER Timeout OPTIONAL );

// from bus.c
VOID STDCALL BusAddTarget ( IN PUCHAR ClientMac, IN PUCHAR ServerMac,
...
NTSTATUS STDCALL AoEStart (  )
{
	...
	DbgPrint ( "AoEStart\n" );
	if ( ( ProbeTag =
	       ( PTAG ) ExAllocatePool ( NonPagedPool,
					 sizeof ( TAG ) ) ) == NULL ) {
...
VOID STDCALL AoEStop (  )
{
	...
	while ( Tag != NULL ) {
		if ( Tag->Request != NULL && --Tag->Request->TagCount == 0 ) {
			...
			IoCompleteRequest ( Tag->Request->Irp,
					    IO_NO_INCREMENT );
...
BOOLEAN STDCALL AoESearchDrive ( IN PDEVICEEXTENSION DeviceExtension )
{
	...
	if ( ( DiskSearch =
	       ( PDISKSEARCH ) ExAllocatePool ( NonPagedPool,
						sizeof ( DISKSEARCH ) ) ) ==
	     NULL ) {
	...
	while ( TRUE ) {
		...
		KeWaitForSingleObject ( &DeviceExtension->Disk.SearchEvent,
		...
		if ( DeviceExtension->Disk.SearchState == SearchNIC ) {
			if ( !ProtocolSearchNIC
			     ( DeviceExtension->Disk.ClientMac ) ) {
				KeReleaseSpinLock ( &DeviceExtension->Disk.
						    SpinLock, Irql );
				continue;
			} else {
				DeviceExtension->Disk.MTU =
				    ProtocolGetMTU ( DeviceExtension->Disk.
						     ClientMac );
		...
		if ( DeviceExtension->Disk.SearchState == GettingSize ) {
			KeReleaseSpinLock ( &DeviceExtension->Disk.SpinLock,
					    Irql );
			continue;
		}
		...
		if ( DeviceExtension->Disk.SearchState ==
			...
			if ( CurrentTime.QuadPart > MaxSectorsPerPacketSendTime.QuadPart + 2500000LL ) {	// 2.500.000 * 100ns = 250.000.000 ns = 250ms
				DbgPrint
				    ( "No reply after 250ms for MaxSectorsPerPacket %d, giving up\n",
		...
		if ( DeviceExtension->Disk.SearchState == Done ) {
			...
			if ( TagWalker != NULL ) {
				...
				if ( OutstandingTags < 0 )
					DbgPrint
					    ( "!!OutstandingTags < 0 (AoESearchDrive)!!\n" );
				ExFreePool ( Tag->PacketData );
				ExFreePool ( Tag );
			}
			...
			if ( DiskSearchList == NULL ) {
				...
				if ( DiskSearchWalker ) {
					if ( DiskSearchWalker ==
						...
						PreviousDiskSearch->Next =
						    DiskSearchWalker->Next;
					}
					ExFreePool ( DiskSearchWalker );
				} else {
					DbgPrint
					    ( "!!Disk not found in DiskSearchList!!\n" );
				}
			}
			...
			KeReleaseSpinLock ( &DeviceExtension->Disk.SpinLock,
					    Irql );
			DbgPrint
			    ( "Disk size: %I64uM cylinders: %I64u heads: %u sectors: %u sectors per packet: %u\n",
		...
		if ( ( Tag->PacketData =
		       ( PAOE ) ExAllocatePool ( NonPagedPool,
						 Tag->PacketSize ) ) == NULL ) {
			DbgPrint
			    ( "AoESearchBootDrive ExAllocatePool Tag->PacketData\n" );
			ExFreePool ( Tag );
			Tag = NULL;
			KeReleaseSpinLock ( &DeviceExtension->Disk.SpinLock,
					    Irql );
		...
		case GetMaxSectorsPerPacket:
			...
			Tag->PacketData->Count =
			    ( UCHAR ) ( ++DeviceExtension->Disk.
					MaxSectorsPerPacket );
			KeQuerySystemTime ( &MaxSectorsPerPacketSendTime );
			DeviceExtension->Disk.SearchState =
			    GettingMaxSectorsPerPacket;
			DeviceExtension->Disk.Timeout = 200000;
			break;
		default:
			DbgPrint
			    ( "AoESearchBootDrive Undefined SearchState!\n" );
			ExFreePool ( Tag->PacketData );
			ExFreePool ( Tag );
			KeReleaseSpinLock ( &DeviceExtension->Disk.SpinLock,
					    Irql );
			      ...
			      IN PIRP Irp )
{
	...
	if ( ( Request =
	       ( PREQUEST ) ExAllocatePool ( NonPagedPool,
					     sizeof ( REQUEST ) ) ) == NULL ) {
	...
	for ( i = 0; i < SectorCount;
		...
		if ( ( Tag->PacketData =
		       ( PAOE ) ExAllocatePool ( NonPagedPool,
						 Tag->PacketSize ) ) == NULL ) {
			DbgPrint
			    ( "AoERequest ExAllocatePool Tag->PacketData\n" );
			ExFreePool ( Tag );
			Tag = NewTagList;
			while ( Tag != NULL ) {
				PreviousTag = Tag;
		...
		if ( Mode == Write )
			RtlCopyMemory ( Tag->PacketData->Data,
			    ...
			    IN PUCHAR Data, IN UINT DataSize )
{
	...
	if ( ProbeTag->Id == Reply->Tag ) {
		RtlCopyMemory ( &LBASize, &Reply->Data[200],
				sizeof ( LONGLONG ) );
		BusAddTarget ( DestinationMac, SourceMac,
			       ntohs ( Reply->Major ), Reply->Minor, LBASize );
		return STATUS_SUCCESS;
	}
	...
	if ( RtlCompareMemory
	     ( Tag->DeviceExtension->Disk.ServerMac, "\xff\xff\xff\xff\xff\xff",
	       6 ) == 6 ) {
		RtlCopyMemory ( Tag->DeviceExtension->Disk.ServerMac, SourceMac,
				6 );
		DbgPrint
		    ( "Major: %d minor: %d found on server %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	case SearchDriveType:
		KeAcquireSpinLock ( &Tag->DeviceExtension->Disk.SpinLock,
				    &Irql );
		...
		case GettingMaxSectorsPerPacket:
			...
			if ( DataSize <
			     ( Tag->DeviceExtension->Disk.MaxSectorsPerPacket *
			       SECTORSIZE ) ) {
				DbgPrint
				    ( "AoEReply Packet size too low while getting MaxSectorsPerPacket (tried %d, got size of %d)\n",
				      Tag->DeviceExtension->Disk.
				      MaxSectorsPerPacket, DataSize );
				Tag->DeviceExtension->Disk.
				    MaxSectorsPerPacket--;
				Tag->DeviceExtension->Disk.SearchState = Done;
				    ...
				    ( sizeof ( AOE ) +
					  ...
					  1 ) * SECTORSIZE ) ) ) {
				DbgPrint
				    ( "AoEReply Got MaxSectorsPerPacket %d at size of %d. MTU of %d reached\n",
				      Tag->DeviceExtension->Disk.
				      MaxSectorsPerPacket, DataSize,
				...
				DbgPrint
				    ( "AoEReply Got MaxSectorsPerPacket %d at size of %d, trying next...\n",
				      Tag->DeviceExtension->Disk.
				      MaxSectorsPerPacket, DataSize );
				Tag->DeviceExtension->Disk.SearchState =
				    GetMaxSectorsPerPacket;
		...
		KeReleaseSpinLock ( &Tag->DeviceExtension->Disk.SpinLock,
				    Irql );
		KeSetEvent ( &Tag->DeviceExtension->Disk.SearchEvent, 0,
			     FALSE );
		break;
	case RequestType:
		if ( Tag->Request->Mode == Read )
			RtlCopyMemory ( &Tag->Request->
					Buffer[Tag->BufferOffset], Reply->Data,
					Tag->SectorCount * SECTORSIZE );
...
VOID STDCALL Thread ( IN PVOID StartContext )
{
	...
	while ( TRUE ) {
		...
		KeWaitForSingleObject ( &ThreadSignalEvent, Executive,
		...
		if ( CurrentTime.QuadPart >
		     ( ReportTime.QuadPart + 10000000LL ) ) {
