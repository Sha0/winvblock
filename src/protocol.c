			       ...
			       IN PVOID PacketContext )
{
	...
	if ( RtlCompareMemory ( SourceMac, "\xff\xff\xff\xff\xff\xff", 6 ) ==
	     6 ) {
		while ( Context != NULL ) {
					    ...
					    IN UINT BytesTransferred )
{
	...
	if ( Header != NULL && Data != NULL )
		AoEReply ( Header->SourceMac, Header->DestinationMac, Data,
			   DataSize );
	if ( Header != NULL )
		ExFreePool ( Header );
	if ( Data != NULL )
				      ...
				      IN UINT PacketSize )
{
	...
	if ( !NT_SUCCESS ( Status ) ) {
		Error ( "ProtocolReceive NdisAllocateBuffer (HeaderCopy)",
			Status );
		NdisUnchainBufferAtFront ( Packet, &Buffer );
		NdisFreeBuffer ( Buffer );
		NdisFreePacket ( Packet );
				   ...
				   IN PVOID SystemSpecific2 )
{
	...
	NdisAllocateBufferPool ( &Status, &Context->BufferPoolHandle,
				 POOLSIZE );
	if ( !NT_SUCCESS ( Status ) ) {
	...
	if ( NT_SUCCESS
		...
		if ( ( Context->AdapterName =
		     ...
		     NULL ) {
			DbgPrint
			    ( "ProtocolBindAdapter ExAllocatePool AdapterName\n" );
		} else {
		...
		Error ( "ProtocolBindAdapter NdisQueryAdapterInstanceName",
			Status );
	}
	...
	if ( DeviceName->Length > 0 ) {
		if ( ( Context->DeviceName =
		     ...
		     NULL ) {
			DbgPrint
			    ( "ProtocolBindAdapter ExAllocatePool DeviceName\n" );
		} else {
