				...
				IN PDEVICE_OBJECT PhysicalDeviceObject )
{
	...
	if ( !PhysicalMemory ) {
		...
		for ( Offset = 0; Offset < 0xa0000; Offset += 0x10 ) {
			if ( ( ( PABFT ) & PhysicalMemory[Offset] )->
			     Signature == 0x54464261 ) {
				Checksum = 0;
				for ( i = 0;
				      i <
				      ( ( PABFT ) & PhysicalMemory[Offset] )->
				      Length; i++ )
				...
				if ( ( ( PABFT ) & PhysicalMemory[Offset] )->
				     Revision != 1 ) {
					DbgPrint
					    ( "Found aBFT with mismatched revision v%d at segment 0x%4x. want v1.\n",
					      ( ( PABFT ) &
						PhysicalMemory[Offset] )->
					      Revision, ( Offset / 0x10 ) );
					continue;
				}
				DbgPrint ( "Found aBFT at segment: 0x%04x\n",
					   ( Offset / 0x10 ) );
				RtlCopyMemory ( &AOEBootRecord,
						&PhysicalMemory[Offset],
						sizeof ( ABFT ) );
				FoundAbft = TRUE;
				break;
...
#ifdef RIS
	FoundAbft = TRUE;
	RtlCopyMemory ( AOEBootRecord.ClientMac, "\x00\x0c\x29\x34\x69\x34",
			6 );
	...
	if ( FoundAbft ) {
		DbgPrint
		    ( "Boot from client NIC %02x:%02x:%02x:%02x:%02x:%02x to major: %d minor: %d\n",
		...
		if ( !BusAddChild
		     ( DeviceObject, AOEBootRecord.ClientMac,
		       AOEBootRecord.Major, AOEBootRecord.Minor, TRUE ) ) {
			    ...
			    USHORT Major, UCHAR Minor, LONGLONG LBASize )
{
	...
	while ( Walker != NULL ) {
		if ( ( RtlCompareMemory
		       ( &Walker->Target.ClientMac, ClientMac, 6 ) == 6 )
		     &&
		     ( RtlCompareMemory
		       ( &Walker->Target.ServerMac, ServerMac, 6 ) == 6 )
		     && Walker->Target.Major == Major
		     && Walker->Target.Minor == Minor ) {
			if ( Walker->Target.LBASize != LBASize ) {
				DbgPrint
				    ( "BusAddTarget LBASize changed for e%d.%d (%I64u->%I64u)\n",
				      Major, Minor, Walker->Target.LBASize,
				      LBASize );
			      ...
			      IN ULONG Minor, IN BOOLEAN Boot )
{
	...
	KeInitializeEvent ( &DeviceExtension->Disk.SearchEvent,
	...
	DeviceExtension->Disk.DiskNumber =
	    InterlockedIncrement ( &NextDisk ) - 1;
				  ...
				  IN PDEVICEEXTENSION DeviceExtension )
{
	...
	case IRP_MN_START_DEVICE:
		...
		IoSetCompletionRoutine ( Irp,
					 ...
					 IoCompletionRoutine, ( PVOID ) & Event,
					 TRUE, TRUE, TRUE );
		Status =
		    IoCallDriver ( DeviceExtension->Bus.LowerDeviceObject,
				   Irp );
					    ...
					    DeviceExtension )
{
	...
	case IOCTL_AOE_SCAN:
		...
		if ( ( Targets =
		     ...
		     == NULL ) {
			DbgPrint
			    ( "BusDispatchDeviceControl ExAllocatePool Targets\n" );
			Irp->IoStatus.Information = 0;
			Status = STATUS_INSUFFICIENT_RESOURCES;
			break;
		}
		...
		TargetWalker = TargetList;
		while ( TargetWalker != NULL ) {
			RtlCopyMemory ( &Targets->Target[Count],
					&TargetWalker->Target,
					sizeof ( TARGET ) );
			Count++;
			TargetWalker = TargetWalker->Next;
	...
	case IOCTL_AOE_SHOW:
		...
		if ( ( Disks =
		     ...
		     NULL ) {
			DbgPrint
			    ( "BusDispatchDeviceControl ExAllocatePool Disks\n" );
			Irp->IoStatus.Information = 0;
			Status = STATUS_INSUFFICIENT_RESOURCES;
			break;
		}
		...
		while ( DiskWalker != NULL ) {
			...
			Disks->Disk[Count].LBASize =
			    DiskWalker->Disk.LBADiskSize;
			Count++;
			DiskWalker = DiskWalker->Disk.Next;
		}
	...
	case IOCTL_AOE_MOUNT:
		...
		DbgPrint
		    ( "Got IOCTL_AOE_MOUNT for client: %02x:%02x:%02x:%02x:%02x:%02x Major:%d Minor:%d\n",
		      Buffer[0], Buffer[1], Buffer[2], Buffer[3], Buffer[4],
		      Buffer[5], *( PUSHORT ) ( &Buffer[6] ),
		      ( UCHAR ) Buffer[8] );
		if ( !BusAddChild
	...
	case IOCTL_AOE_UMOUNT:
		...
		if ( DiskWalker != NULL ) {
			...
			if ( DiskWalker == DeviceExtension->Bus.ChildList ) {
				DeviceExtension->Bus.ChildList =
				    DiskWalker->Disk.Next;
			} else {
				DiskWalkerPrevious->Disk.Next =
				    DiskWalker->Disk.Next;
			}
