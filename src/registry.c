...
BOOLEAN STDCALL SetupRegistry ( OUT PNTSTATUS StatusOut )
{
    ...
    PWCHAR InterfacesKeyString, LinkageKeyString, NdiKeyString,
	DriverServiceNameString, NewValue;
    UNICODE_STRING NetworkClassKey, InterfacesKey, LinkageKey, NdiKey,
	LowerRange, UpperBind, Service, DriverServiceName;
    ...
    InitializeObjectAttributes ( &NetworkClassKeyObject, &NetworkClassKey,
...
// start nic (
	    NdiKeyStringLength =
		KeyInformation->NameLength + sizeof ( NdiPath );
	    if ( ( NdiKeyString =
		   ( PWCHAR ) ExAllocatePool ( NonPagedPool,
					       NdiKeyStringLength ) ) ==
		 NULL ) {
	    ...
	    if ( NT_SUCCESS
		 ( ZwOpenKey
		   ( &SubKeyHandle, KEY_ALL_ACCESS, &SubKeyObject ) ) ) {
		if ( ( Status =
