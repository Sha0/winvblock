...
int _main ( t_cpu * cpu )
{
	...
	memset ( ( void * )&undigetinformation, 0,
		 sizeof ( undigetinformation ) );
	if ( api ( PXENV_UNDI_GET_INFORMATION, &undigetinformation ) )
		apierror ( "PXENV_UNDI_GET_INFORMATION",
			   undigetinformation.Status );
	...
	printf
	    ( "Target: e%d.%d  Size: %dM  Cylinders: %d  Heads: %d  Sectors: %d\n\n",
	      major, minor, ( ( disksize / 2 ) / 1024 ), cylinders, heads,
	      sectors );
	do {
...
int getbootinfo (  )
{
	...
	segmemcpy ( ( ( segment << 16 ) + ( int )&info ),
		    ( ( getinfo.Buffer.segment << 16 ) +
		      getinfo.Buffer.offset ), sizeof ( info ) );
...
void getbootdriveinfo (  )
{
	...
	sendreceive ( send, sizeof ( t_aoe ) - ( 2 * SECTORSIZE ), receive,
		      &i );
...
void _int13 ( t_cpu * cpu )
{
	...
	case 0x03:
		if ( ( cpu->edx & 0xff ) != aoedrive )
			goto unhandled;
		cylinder =
		    ( ( cpu->ecx >> 8 ) & 0xff ) + ( ( cpu->ecx & 0xc0 ) << 2 );
		head = ( cpu->edx >> 8 ) & 0xff;
		sector = cpu->ecx & 0x3f;
		lba =
		    ( ( ( cylinder * heads ) + head ) * sectors ) + sector - 1;
		readwrite ( ( ( ( cpu->eax >> 8 ) & 0xff ) == 0x02 ? 1 : 0 ),
			    lba, cpu->eax & 0xff, cpu->es, cpu->ebx & 0xffff );
		cpu->eax &= ~( 0xff << 8 );
		cpu->eflags &= ~1;
		break;
	...
	case 0x43:
		...
		readwrite ( ( ( ( cpu->eax >> 8 ) & 0xff ) == 0x42 ? 1 : 0 ),
			    rwpacket.lba, rwpacket.count,
			    rwpacket.buffersegment, rwpacket.bufferoffset );
		cpu->eax &= ~( 0xff << 8 );
		cpu->eflags &= ~1;
		break;
...
//    send->lba5 = (s >> 40) & 255;
		...
		sendreceive ( send, sizeof ( t_aoe ) - ( 2 * SECTORSIZE ),
			      receive, &i );

		if ( read ) {
			segmemcpy ( ( ( seg << 16 ) + off ),
				    ( ( segment << 16 ) +
				      ( int )receive->data ), c * SECTORSIZE );
		}
...
int processpacket ( void *buffer, int *size )
{
	...
	if ( isr.FuncFlag == PXENV_UNDI_ISR_OUT_RECEIVE ) {
		...
		*size = isr.FrameLength;
		segmemcpy ( ( ( segment << 16 ) + ( int )buffer +
			      receiveddata ),
