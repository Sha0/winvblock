...
int main ( int argc, char **argv, char **envp )
{
	...
	if ( Targets->Count == 0 ) {
		...
		sprintf ( String, "e%lu.%lu      ",
			  Targets->Target[i].Major, Targets->Target[i].Minor );
		String[10] = 0;
		printf
	...
	if ( Disks->Count == 0 ) {
		...
		sprintf ( String, "e%lu.%lu      ",
			  Disks->Disk[i].Major, Disks->Disk[i].Minor );
		String[10] = 0;
		printf
		    ( " %-4lu %02x:%02x:%02x:%02x:%02x:%02x  %02x:%02x:%02x:%02x:%02x:%02x  %s  %I64uM\n",
		      Disks->Disk[i].Disk,
		      Disks->Disk[i].ClientMac[0],
		      Disks->Disk[i].ClientMac[1],
		      Disks->Disk[i].ClientMac[2],
		      Disks->Disk[i].ClientMac[3],
		      Disks->Disk[i].ClientMac[4],
		      Disks->Disk[i].ClientMac[5],
		      Disks->Disk[i].ServerMac[0],
		      Disks->Disk[i].ServerMac[1],
		      Disks->Disk[i].ServerMac[2],
		      Disks->Disk[i].ServerMac[3],
		      Disks->Disk[i].ServerMac[4],
	...
	*( PUCHAR ) ( &InBuffer[8] ) = ( UCHAR ) Minor;
	...
	if ( !DeviceIoControl
	     ( DeviceHandle, IOCTL_AOE_UMOUNT, InBuffer,
	       sizeof ( InBuffer ), NULL, 0, &BytesReturned,
	       ( LPOVERLAPPED ) NULL ) ) {
	    printf ( "DeviceIoControl (%d)\n", ( int )GetLastError (  ) );
	}
