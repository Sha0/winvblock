...
/* in this file */
static NTSTATUS STDCALL BusGetDeviceCapabilities ( IN PDEVICE_OBJECT DeviceObject,
				   ...
				   IN PDEVICEEXTENSION DeviceExtension )
{
	...
	case BusQueryDeviceID:
	    ...
	    RtlCopyMemory ( ( PWCHAR ) Irp->IoStatus.Information,
			    String, StringLength * sizeof ( WCHAR ) );
				    ...
				    IN PDEVICEEXTENSION DeviceExtension )
{
	...
	case SRB_FUNCTION_EXECUTE_SCSI:
		...
		if ( Cdb->AsByte[0] == SCSIOP_READ16
		    ...
		    REVERSE_BYTES_QUAD ( &StartSector, &( ( ( PCDB16 )
							    Cdb )->LogicalBlock
							  [0] ) );
		    REVERSE_BYTES ( &SectorCount,
				    &( ( ( PCDB16 ) Cdb )->TransferLength
				       [0] ) );
		} else {
		    StartSector =
		...
		if ( ( ( ( PUCHAR ) Srb->DataBuffer - ( PUCHAR )
 ...
 */
		...
		Temp = SECTORSIZE;
		REVERSE_BYTES ( &( ( ( PREAD_CAPACITY_DATA )
		...
		Temp = SECTORSIZE;
		REVERSE_BYTES ( &( ( ( PREAD_CAPACITY_DATA_EX )
				     Srb->DataBuffer )->BytesPerBlock ),
				&Temp );
