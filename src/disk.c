				   ...
				   IN PDEVICEEXTENSION DeviceExtension )
{
	...
	case IRP_MN_QUERY_ID:
		if ( ( String =
		     ...
		     NULL ) {
			DbgPrint
			    ( "DiskDispatchPnP ExAllocatePool IRP_MN_QUERY_ID\n" );
		...
		case BusQueryDeviceID:
			StringLength =
			    swprintf ( String, L"AoE\\e%d.%d",
				       DeviceExtension->Disk.Major,
				       DeviceExtension->Disk.Minor ) + 1;
			...
			RtlCopyMemory ( ( PWCHAR ) Irp->IoStatus.Information,
					String,
...
NTSTATUS STDCALL DiskDispatchSCSI ( IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp,
				    IN PIO_STACK_LOCATION Stack,
				    IN PDEVICEEXTENSION DeviceExtension )
{
	NTSTATUS Status;
		...
		case SRB_FUNCTION_EXECUTE_SCSI:
			...
			case SCSIOP_WRITE16:
				if ( Cdb->AsByte[0] == SCSIOP_READ16
					...
					StartSector =
					    ( Cdb->CDB10.
					      LogicalBlockByte0 << 24 ) +
					    ( Cdb->CDB10.
					      LogicalBlockByte1 << 16 ) +
					    ( Cdb->CDB10.
					      LogicalBlockByte2 << 8 ) +
				...
				if ( ( StartSector + SectorCount >
					...
					SectorCount =
					    ( ULONG ) ( DeviceExtension->Disk.
							LBADiskSize -
							StartSector );
				}
				if ( SectorCount * SECTORSIZE >
				...
				if ( Srb->DataTransferLength % SECTORSIZE != 0 )
					DbgPrint
					    ( "!! DataTransferLength not aligned !!\n" );
				if ( Srb->DataTransferLength >
				     SectorCount * SECTORSIZE )
					DbgPrint
					    ( "!! DataTransferLength too big !!\n" );
				...
				if ( Cdb->AsByte[0] == SCSIOP_READ
					...
					return AoERequest ( DeviceExtension,
							    Read, StartSector,
							    SectorCount,
							    ( ( PUCHAR ) Srb->
							      DataBuffer -
			...
			case SCSIOP_READ_CAPACITY:
				...
				REVERSE_BYTES ( &
						( ( ( PREAD_CAPACITY_DATA )
						    Srb->DataBuffer )->
						  BytesPerBlock ), &Temp );
				if ( ( DeviceExtension->Disk.LBADiskSize - 1 ) >
				     0xffffffff ) {
	       ...
	       -1;
					...
					Temp =
					    ( ULONG ) ( DeviceExtension->Disk.
							LBADiskSize - 1 );
					REVERSE_BYTES ( &
							( ( ( PREAD_CAPACITY_DATA ) Srb->DataBuffer )->LogicalBlockAddress ), &Temp );
				}
			...
			case SCSIOP_READ_CAPACITY16:
				...
				REVERSE_BYTES ( &
						( ( ( PREAD_CAPACITY_DATA_EX )
						  ...
						  BytesPerBlock ), &Temp );
				LargeTemp =
				    DeviceExtension->Disk.LBADiskSize - 1;
				REVERSE_BYTES_QUAD ( &
						     ( ( ( PREAD_CAPACITY_DATA_EX ) Srb->DataBuffer )->LogicalBlockAddress.QuadPart ), &LargeTemp );
				Irp->IoStatus.Information =
...
//        StorageAdapterDescriptor.MaximumTransferLength = SECTORSIZE * POOLSIZE;
		...
		if ( StoragePropertyQuery->PropertyId == StorageDeviceProperty
			...
			StorageDeviceDescriptor.Size =
			    sizeof ( STORAGE_DEVICE_DESCRIPTOR );
			StorageDeviceDescriptor.DeviceType =
	...
	case IOCTL_SCSI_GET_ADDRESS:
		...
		ScsiAdress.PathId = 0;
		ScsiAdress.TargetId =
		    ( UCHAR ) DeviceExtension->Disk.DiskNumber;
		ScsiAdress.Lun = 0;
					    ...
					    DeviceCapabilities )
{
	...
	pnpIrp =
	    IoBuildSynchronousFsdRequest ( IRP_MJ_PNP, targetObject, NULL, 0,
					   NULL, &pnpEvent, &ioStatus );
	if ( pnpIrp == NULL ) {
		status = STATUS_INSUFFICIENT_RESOURCES;
	} else {
		...
		if ( status == STATUS_PENDING ) {
			KeWaitForSingleObject ( &pnpEvent, Executive,
